...
| D3 | 서비스 루트: `~/Projects/paper-review-service/<slug>/` | 1 paper = 1 Claude Code 프로젝트. `cd` 만 하면 `~/.claude/projects/...` 에 세션이 자동 그룹화 |
...
```
~/Projects/paper-review-service/                       # 서비스 루트
├── DESIGN.md                           # ← 이 문서
...
2. `~/Projects/paper-review-service/<slug>/` 생성
...
**Input**: `~/Projects/paper-review-service/<slug>/workbench.md` (+ paper.json + figures/)
...
paper-review session <slug>            # cd ~/Projects/paper-review-service/<slug> && claude
...
**수정**
- 출력 경로 하드코딩 (`/mnt/user-data/outputs/`, `/tmp/papers/`) → `~/Projects/paper-review-service/<slug>/` 기반으로 환경 변수 또는 인자
...
- 5-7개를 `~/Projects/paper-review-service/src/paper_review/publish/voice_samples/` 에 정적 저장
