...
### Added
- **Summary를 HTML로 내보낸다.** 내보내기 버튼을 Summary 화면에서 누르면
  "PDF로 내보내기 / HTML로 내보내기" 메뉴가 뜬다. HTML은 **그림을 파일 안에 넣은
  단독 파일**이라 메일로 보내거나 서버 없이 열어도 그대로 보이고, 접기 동작과 가독성
  보정도 유지된다. 리포트 오른쪽 위 ⤓도 같은 파일을 받는다. 예전 ⤓ 파일은 그림이
  서버 주소를 가리켜서 다른 곳에서 열면 전부 깨졌다(13편 중 10편이 해당).
- 데스크톱 앱에서 다운로드를 허용했다(pywebview 기본값은 꺼짐). HTML은 앱이 창에
  그냥 열어 버리므로, 내보내기는 다운로드 형식으로 보내 저장 창이 뜨게 한다.

### Fixed
- **PDF로 내보내면 접힌 내용이 빠지던 문제.** Summary의 "깊게 —", "내 정리 —" 같은
  접힌 블록은 종이에서 펼칠 수 없어 제목만 인쇄됐다. 이제 인쇄 직전에 모든 블록을
  펼치고, 인쇄가 끝나면 화면은 원래 상태로 되돌린다. (intonation 리포트 18쪽 → 20쪽)

## 2.27.2

### Fixed
- **Summary 글자가 배경과 비슷해 읽기 어렵던 문제 — 화면과 다운로드한 PDF 모두.**
  리포트마다 모델이 색 변수 이름을 새로 짓기 때문에(`--fg-dim`, `--fg-mute` …)
  2.22.0의 정본 팔레트가 이름을 맞히지 못하면 그대로 방치됐다. 예: intonation 리포트는
  라이트 블록이 배경만 바꾸고 글자색은 다크 값으로 남겨, 흰 배경에 연한 글자가 올라갔다.
  이제 변수 이름을 쫓지 않고, **렌더된 글자와 실제 배경의 명암비를 계산해 WCAG AA
  (4.5:1) 미만인 글자만** 색조를 유지한 채 읽히는 쪽으로 보정한다. 측정: 이 리포트의
  기준 미달 글자 835개 중 라이트 69 → 0, 다크 61 → 0.
- **PDF가 화면보다 훨씬 흐리던 문제.** 인쇄 시 브라우저가 카드·강조 박스의 배경색을
  빼 버려, 어두운 배경에 맞춘 밝은 글자가 흰 종이에 남았다. 이제 인쇄(PDF)는 앱 테마와
  무관하게 라이트로 내보내고 배경색도 화면 그대로 인쇄한다.
- 새로 생성되는 리포트는 두 테마 팔레트를 빠짐없이 정의하고 명암비 4.5:1 이상을
  지키도록 생성 프롬프트에 명시했다.

## 2.27.1

### Fixed
- **재생성 도중에 보낸 Summary 요청이 반영되지 않은 채 사라지던 문제.** 재생성이
  성공하면 `report-requests.md`를 통째로 `.history/`로 옮겼는데, 재생성이 파일을
  읽은 **뒤에** 채팅으로 들어온 요청까지 함께 옮겨졌다. 이제 재생성이 읽은 내용만
  보관하고, 그 뒤에 추가된 요청은 남겨 다음 재생성에 반영한다.

## 2.27.0

### Added
- **Summary를 보면서 Claude와 대화하고, 재생성 때 반영한다.** Summary 화면에서
  채팅을 보내면 리포트(`report.html`·`report.md`)와 원문 리뷰를 읽고 답하지만
  **파일은 고치지 않는다** — `Edit`·`Write`를 막아 둔다. 대화의 각 턴은 논문 폴더의
  `report-requests.md`에 기록되고, **Regenerate Report**를 누르면 그 요청을 모두
  반영해 리포트를 다시 만든다. 반영이 끝난 요청은 `.history/`로 옮겨 다음 재생성에
  중복 적용되지 않는다.
  - 재생성 버튼에 반영 대기 요청 수가 뜨고(`Regenerate Report · 요청 2건`),
    확인창에도 반영할 요청 수를 적는다.
  - 채팅 헤더에 "Summary 논의 · 재생성 시 반영"을 표시해 무엇이 바뀌는지 분명히 한다.
  - Summary 대화와 리뷰 대화는 **세션을 분리**한다. 폴더의 최근 세션을 잇는
    `--continue`로는 둘이 섞여 "기재해줘"가 엉뚱한 문서로 갈 수 있었다.
  - 리뷰(Detail) 화면 채팅은 기존과 같다.

## 2.26.0

### Added
- **모든 등록 형식에서 figure를 자동 추출한다.** 지금까지 figure 추출은 arXiv
  전용(ar5iv·e-print)이라, PDF 파일로 등록한 논문은 `figures.json` 자체가 없었다
  (라이브러리 기준 약 100편). 이제 PDF에서도 뽑는다 — 캡션(`Figure 3:`,
  `Table 2`)을 찾아 그 캡션에 속한 그래픽·표 가로선의 범위를 정하고 그 영역만
  렌더한다. 벡터로 그린 차트도 나온다(PDF에 박힌 이미지만 꺼내는 방식으로는
  Qwen 논문에서 한 장도 못 건졌다 — 큰 이미지 객체가 0개였다).
  arXiv 논문도 ar5iv·tarball이 0개면 받아둔 PDF로 폴백한다.
  표본 10편 측정: 8편에서 캡션 번호 누락 0개, Qwen 논문은 figure 13·table 11로
  본문 캡션 수와 일치, MDPI는 0 → 7, Elsevier 표는 0 → 9, 가로선 없는 표(KCI)도
  텍스트 블록으로 잡는다. 논문 한 편당 0.5~1초, 새 의존성 없음(pypdfium2 사용).

### Known limitations
- 나란히 놓인 float(왼쪽 figure + 오른쪽 표)는 한 영역으로 잘릴 수 있다.
- 같은 칼럼에 연달아 붙은 표는 경계가 어긋나는 경우가 있다.
...
### Fixed
- **분석이 실패해도 해결 방법을 알려주지 않던 문제.** 로그인 만료 안내 자체는
  있었지만 **stderr만 검사**하고 있었다 — CLI는 만료를 JSON 스트림으로 알려서
  stderr가 비어 있었고(`✗ exit 1:` 뒤에 아무것도 없던 이유), 안내는 한 번도
  뜨지 않았다. 이제 스트림에 찍힌 오류까지 함께 보고 판단한다.
- **같은 이유로 실패할 섹션을 계속 시도하던 문제.** 로그인이 만료되면 남은 섹션이
  전부 똑같이 실패한다(스크린샷의 7/17, 8/17…). 원인이 재시도로 풀리지 않는
  종류면 그 자리에서 중단하고 남은 개수를 알린다.

### Added
- **실패 원인과 해결 절차를 화면에 표시한다.** 진행 토스트와 분석 로그 창에
  "무엇이 막혔는지 + 번호 매긴 해결 단계 + 명령어 복사 버튼"이 뜬다. 현재 3종:
  로그인 만료(`claude auth login`), 사용량 한도, `claude` 명령 없음.

## 2.24.0

### Added
- **자바스크립트로 그리는 페이지도 등록된다.** 본문을 클라이언트에서 렌더하는
  블로그(qwen.ai/blog 등)는 받아온 HTML에 글이 없어 등록이 실패했다. 이제 그
  경우에만 창 없는 `WKWebView`로 페이지를 렌더한 뒤 기존 추출 경로를 태운다.
  PyObjC는 이미 pywebview로 들어와 있어 **새 의존성이 없고**, 앱과 서버 사이
  왕복도 없다(등록 프로세스 안에서 직접 렌더). 정상 페이지는 렌더를 타지 않아
  등록 시간이 그대로다. 렌더 실패는 기존 안내 메시지로 떨어진다.
  qwen.ai 기준 본문 4자 → **25,657자 · 23섹션 · 그림 5장**, 약 8초.
- 렌더 시 쿠키·로컬스토리지를 공유하지 않는 임시 데이터 스토어를 쓴다 — 남의
  자바스크립트를 실행하는 자리이므로 우리 세션이 새어 나가지 않는다.

### Fixed
- **SPA 등록 시 제목·날짜가 앱 껍데기 값으로 잡히던 문제.** `<title>`과 og:* 는
  포스트가 아니라 앱을 가리킨다(qwen.ai는 "Qwen Studio", 날짜는 3주 차이). 렌더
  경로에서는 글의 `<h1>`을 제목으로 쓰고, 믿을 수 없는 날짜는 비운다.
- **`/blog?id=...` 형태가 일반 article로 분류되던 문제.** 힌트 패턴이 `/blog/`
  처럼 뒤 슬래시를 요구했다. 이제 blog-review 스킬로 올바로 간다.

## 2.23.1

### Fixed
- **로그 복사 버튼이 보이지 않던 문제.** 버튼이 스크롤되는 로그 박스 *안에* 절대
  위치로 붙어 있어서, 로그가 새 줄마다 맨 아래로 자동 스크롤되면 위로 밀려
  사라졌다(DOM에는 있었지만 화면에서 볼 수 없었다). 박스를 감싸는 래퍼에 붙여
  스크롤과 무관하게 항상 보인다.
- **등록 실패 시 이유 대신 파이썬 트레이스백이 나오던 문제.** `_run`이
  `check=True`라 `CalledProcessError`가 먼저 터져서, 스크립트가 남긴 설명은
  캡처된 채 버려지고 `returned non-zero exit status 1`만 남았다. 이제 스크립트의
  메시지를 그대로 올려 CLI가 `✗ <이유>` 한 줄로 출력한다(앱의 등록 로그에 보이는
  것이 이 줄이다). 실패한 `_staging`도 함께 정리한다.
- **자바스크립트로 렌더되는 페이지에 맞는 안내.** `https://qwen.ai/blog?id=...`
  같은 SPA는 받아온 HTML에 본문이 4자뿐이라 어떤 추출기로도 읽을 수 없는데,
  "no main content found"는 우리 쪽 버그처럼 읽혔다. 이제 그 경우를 구분해
  브라우저에서 PDF로 저장해 등록하라고 안내한다.

## 2.23.0

### Added
- **분석이 그림을 알아서 배치한다.** 지금까지 Detail의 그림은 편집 화면에서 하나씩
  손으로 넣어야 했다. 이제 섹션 하나가 분석될 때마다, 그 섹션이 참조하는 그림을
  본문이 그것을 언급한 문단 바로 뒤에 넣는다(언급이 없으면 해설 끝). 위치 판단은
  이미 데이터에 있다 — 그림마다 `ref_in_section`이 있고 라이브러리 353장 중 291장
  (82%)에 값이 있다. 모델 호출 없이 코드가 하므로 id를 지어낼 수 없다.
  마크다운 형식은 수동 삽입과 동일해 Velog 발행 경로가 그대로 동작한다.
  같은 그림이 이미 있으면 건너뛰므로 analyze를 다시 돌려도 중복되지 않는다.

## 2.22.0

### Fixed
- **Summary 배경색이 생성 시점에 고정되던 문제.** 리포트는 만들어질 때 모델이 쓴
  팔레트를 그대로 안고 있었다 — 라이브러리 8편 기준 다크 우선 5편, 라이트 우선
  2편, 대응이 아예 없는 것 1편. 그나마의 대응도 `prefers-color-scheme`, 즉 **OS**
  설정이라 앱 테마와 무관했다(라이트 앱 안에 다크 리포트가 박혀 있는 이유).
  이제 서버가 리포트를 내보낼 때 정본 팔레트를 주입하고, 리포트가 가진 다크/라이트
  블록은 **앱 테마 속성으로 재조준**한다 — 변수뿐 아니라 리포트가 직접 박아둔
  규칙(nav 배경 등)까지 따라온다. 재생성 없이 기존 리포트 전부에 적용된다.
  Summary 탭·새 창(⧉)·인쇄 화면이 모두 같은 경로를 지난다.

## 2.21.2

### Fixed
- **등록한 논문에 자동 태그가 안 붙던 문제.** 태깅이 `status = "done"` **이후에**
  실행됐는데, 갤러리는 done을 보는 즉시 논문 페이지로 이동하며 폴링을 멈춘다.
  태깅에 20초쯤 걸리므로 그 사이 앱을 닫거나 하면 논문은 태그 없이 남고, 로그에도
  아무 흔적이 남지 않았다(사용자는 태깅이 도는 줄도 몰랐다). 이제 태그를 붙인 뒤에
  done을 보고하며, 진행 중에는 "→ 태그 생성 중 …"이 로그에 보인다.

## 2.21.1

### Fixed
- **arXiv 논문 등록이 실패하던 문제** (`https://arxiv.org/pdf/2608.03893`).
  PDF는 정상적으로 받아지는데 메타데이터를 가져오는 `export.arxiv.org` API가
  timeout·429로 흔들리면 등록 전체가 죽었다. 요청을 3회까지 재시도하고, 그래도
  안 되면 **abs 페이지의 메타 태그에서 제목·저자·초록을 읽는다** — 다른 호스트라
  API가 rate-limit 중이어도 살아 있다. 남의 quota 때문에 이미 받아둔 PDF를
  버리는 것이 이 버그의 실체였다.

## 2.21.0

### Added
- **로그 복사.** 등록 진행 로그와 분석 로그 창에 복사 버튼을 달았다. 실패했을 때
  로그를 밖으로 꺼낼 방법이 없던 것이 가장 큰 문제였다.

### Fixed
- **앱에서 ⌘C가 전혀 동작하지 않던 문제.** macOS는 복사/붙여넣기를 메뉴 항목의
  단축키로 전달하는데 pywebview는 앱 메뉴와 View 메뉴만 만든다 — Edit 메뉴가
  없어서 텍스트를 선택해도 복사가 안 됐다. Edit 메뉴(실행 취소·잘라내기·복사·
  붙여넣기·전체 선택)를 추가해 앱 전체에서 동작한다.

## 2.20.1

### Fixed
- **참고문헌 줄이 없는 논문의 부록이 그대로 섹션이 되던 문제.** 부록 표는 번호가
  1부터 다시 시작하므로 `6 Conclusion` 뒤에 `2 Cross-session`이 나온다 — 번호가
  뒤로 가거나 같은 최상위 번호가 두 번 쓰이면 섹션으로 보지 않는다. 번호를 쓰는
  논문에서 중간에 나오는 대문자 배너(`DECODED REASONING`)도 제외하되, 번호가
  붙은 약어 제목(`4.3.4 STAL`)은 그대로 둔다. 세미콜론으로 끝나는 줄도 제외.
- **필터 체인이 두 곳에 복제돼 있어 `tidy-sections`가 새 규칙을 놓치던 문제.**
  `init_paper.sections()` 한 곳으로 합쳐 ingest와 tidy가 같은 결과를 낸다.
